...
function spawnMsgBox(soundTitle, soundName) {
    ...
    audio.addEventListener('timeupdate', () => {
        progress.innerHTML = `${soundTitle} <br> ${(audio.currentTime / 60).toFixed(2)} <progress value="${
            audio.currentTime
        }" max="${audio.duration}"></progress> ${(audio.duration / 60).toFixed(2)}`;
    });
}
 ...
 */
fetch('sounds.json').then((result) =>
    result.json().then((sounds) => {
        /**
        ...
        if (searchedButtons.length < 1) {
            if (noSearchResult < 5) {
                ...
                document.getElementsByTagName(
                    'main'
                )[0].innerHTML = `<h3 style="margin-top: 10%">Ey ich find halt nix...</h3>`;
            } else {
                document.getElementsByTagName(
                    'main'
                )[0].innerHTML = `<h1 style="margin-top: 10%">Ey verpiss dich einfach 😡!</h1>`;
            }
        } else {
            noSearchResult = 0;
